...
const Card = () => {
  ...
  const getType = async () => {
    const response = await axios.get("http://localhost:8000/backend/api/products");
  ...
  const showname = productinfo?.map((i) => (
    <div key={i.id} className="card">
      ...
      <img
        ...
        src={`http://localhost:8000/backend/storage/products/images/${i.image}`}
        alt="Product Imag"
      />
      <div className="productdetails">
        ...
        <div className="PI">
          ...
          <Link to={`http://localhost:8000/backend/storage/products/pdfs/${i.pdf}`}>
            <img
              className="pdfuser"
