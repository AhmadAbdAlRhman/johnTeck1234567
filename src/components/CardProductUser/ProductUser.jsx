...
const ProductUser = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        if (search) {
          // http://localhost:8000/backend/api/products
          const res = await axios.get(
            `http://localhost:8000/backend/api/products/${search}`
          );
    ...
    <div key={e.id || index} className="Showcard">
      <div className={ i18n.language  === "ar" ?  "C1AR" :"C1user"}>
        <img
          ...
          src={`http://localhost:8000/backend/storage/products/images/${e.image}`}
      ...
      <div className="C2user">
        <div className="productdetailsuser">
          ...
          <div className={ i18n.language === "ar" ? "PIuserAR" : "PIuser"}>
            ...
            <div>
            <a
              href={`http://localhost:8000/backend/storage/products/pdfs/${e.pdf}`}
              target="_blank"
