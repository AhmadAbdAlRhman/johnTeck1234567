...
import { faTrash} from "@fortawesome/free-solid-svg-icons"; // استيراد الأيقونات
...
const ShowCertificateforAdmain = ({cerinfo, getCer}) => {
  ...
  const ShowCer = cerinfo?.map((i) => (
    <div className="cer1" key={i.id}>
      <a
        href={`https://localhost:8000/backend/storage/Certificates/pdf/${i.pdf}`}
        target="_blank"
      ...
      >
        {isLoading ? (
          ...
          <img
            className="cer2"
            src={`https://localhost:8000/backends/storage/Certificates/image/${i.image}`}
