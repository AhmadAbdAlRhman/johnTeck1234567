...
const Certificates = () => {
    ...
    const data = async () => {
      ...
      try {
        const res1 = await axios.get(
          "https://johntekvalves.com/backendapi/certificates"
        );
        SetCert(res1.data);
      } catch (err) {
        console.log(err);
  ...
  const arrcerte = certe.map((e) => (
    <div className="cer1">
      <a
        href={`https://johntekvalves.com/backendstorage/Certificates/pdf/${e.pdf}`}
        target="_blank"
      ...
      >
        {/* {isLoading ? (
          ...
          <img
            className="cer2"
            src={`https://johntekvalves.com/backendstorage/Certificates/image/${e.image}`}
